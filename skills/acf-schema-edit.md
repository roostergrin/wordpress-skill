...
Safely edit ACF field group JSON files by hand, following existing schema patterns, reusing components via clone fields, and maintaining key stability.
...
## Hard Guardrails
- Edit only `*.json` files inside the ACF JSON directory.
...
- Always validate JSON after editing (run `python3 -m json.tool <file>` or `jq empty <file>`).
...
6. Validate: `jq empty < file.json` or `python3 -m json.tool < file.json > /dev/null`
7. **Next step:** deploy using `skills/acf-schema-deploy.md`.

## References
- `acf-schema-edit/references/acf-json-schema-reference.md` — Full ACF JSON schema reference (all field types, all settings)
